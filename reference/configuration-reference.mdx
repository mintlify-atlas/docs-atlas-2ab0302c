...
<Note>
`public_read = false` (the default) keeps silo in the read path for private buckets. Silo streams each file from storage at `/media/<id>`, using the credentials you configured. Nothing else changes and no URL breaks.
</Note>
