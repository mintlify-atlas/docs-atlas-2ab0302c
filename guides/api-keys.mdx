...
| `collections:<p>/<e>/<n>:schema:read` | Read a schema |
| `collections:<p>/<e>/<n>:schema:update` | Update a schema |
...
| `media:configure` | Change media storage settings (bucket, credentials, allowed types) |
...
| `plugins:read` | List plugins |
| `plugins:grant` | Approve plugin claim grants |
| `plugins:enable` | Enable or disable plugins |
| `audit:read` | Read the audit trail |
| `settings:configure` | Change server settings (logging, search, auth, etc.) |
...
<Note>
  ...
  Similarly, `media:configure` is root-only. It exposes credentials and storage endpoints, so it is never carried by plugins and cannot be granted outside of a root-preset key.
</Note>
...
| `manage` | Collection lifecycle, keys, media (including `media:replace`), plugins, settings, observability. Not `media:purge`, `media:configure`, or `settings:configure` |
| `write` | Read + create/update/delete entries, upload and delete media |
| `read` | Read entries, schemas, and search (default for `silo keys create`) |
