...
<Note>
  `PUT` returns `409` if the collection holds entries and the new schema changes which entries are valid. See [Schema validation](#schema-validation) for the full rules on what can and cannot change.
</Note>
